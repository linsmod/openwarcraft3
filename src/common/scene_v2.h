#ifndef __SCENE_V2_H__
#define __SCENE_V2_H__
...
struct scene_t {
    ...
    scene_pause_fn pause;
    scene_resume_fn resume;
    
    // 细粒度事件处理器
    scene_event_handlers_t events;
...
void SceneManager_OnEvent(scene_manager_t *mgr, const scene_event_t *event);

// 事件触发辅助函数
void SceneManager_TriggerEvent(scene_manager_t *mgr, scene_event_type_t type, void* data);
...
// 场景构造宏（新版 - 支持事件处理器）
#define DEFINE_SCENE_WITH_EVENTS(name, init_fn, shutdown_fn, update_fn, render_fn) \
    scene_t name = { \
        .name = #name, \
