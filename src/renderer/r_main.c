...
void R_Init(DWORD width, DWORD height) {
    ...
    // 设置输入法相关hint，允许Shift切换输入法
    SDL_SetHint(SDL_HINT_IME_SHOW_UI, "1");
    SDL_SetHint(SDL_HINT_IME_INTERNAL_EDITING, "0");
    // SDL_HINT_IME_SUPPORT_EXTENDED_TEXT 可能不存在于某些SDL版本
    // SDL_SetHint(SDL_HINT_IME_SUPPORT_EXTENDED_TEXT, "1");
    SDL_SetHint(SDL_HINT_ALLOW_ALT_TAB_WHILE_GRABBED, "1");
    // SDL_HINT_WINDOW_FRAME_USING_PARENT_STYLE 可能不存在于某些SDL版本
    // SDL_SetHint(SDL_HINT_WINDOW_FRAME_USING_PARENT_STYLE, "1");
    SDL_SetHint(SDL_HINT_VIDEO_ALLOW_SCREENSAVER, "1");
    SDL_SetHint(SDL_HINT_GRAB_KEYBOARD, "0");  // 确保不捕获键盘输入
    // SDL_HINT_WINDOW_KEYBOARD_GRABBED 可能不存在于某些SDL版本
    // SDL_SetHint(SDL_HINT_WINDOW_KEYBOARD_GRABBED, "0");
