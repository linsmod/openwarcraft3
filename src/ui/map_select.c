...
static layx_context *g_lay_ctx = NULL;
...
int MapSelect_Init(scene_t *scene) {
    ...
    if (!scene || !scene->manager) {
        printf("Error: scene or scene->manager is NULL\n");
        return -1;
    }
    ...
    // 新的界面布局结构
    // ========================================
    // g_root_container (COLUMN)
    //   ├── title_container (ROW) - 标题
    //   ├── path_container (ROW) - 路径
    //   └── content_container (ROW)
    //        ├── left_container (COLUMN) - 搜索+列表
    //        │    ├── filter_label
    //        │    ├── filter_input
    //        │    └── ui_list
    //        └── right_container (COLUMN) - 预览+按钮
    //             ├── preview_container
    //             │     ├── preview_title_text
    //             │     ├── preview_filename_text
    //             │     ├── preview_name_text
    //             │     ├── preview_author_text
    //             │     ├── preview_players_text
    //             │     ├── preview_type_text
    //             │     └── preview_path_text
    //             └── hint_container (ROW)
    //                  ├── hint_text1
    ...
    // 1. 创建标题容器（第一行）
    ui_component_t *title_container = (ui_component_t *)UIContainer_Create(1024.0f, 40.0f,
        MAKE(COLOR32, 0, 0, 0, 0),  // 透明背景
        MAKE(COLOR32, 0, 0, 0, 0),
        g_ctx
    );
    if (!title_container) {
        printf("Failed to create title container\n");
        return -1;
    }
    UIContainer_AddChild((ui_container_t *)g_root_container, title_container);
    UIComponent_SetSize(title_container, 1024.0f, 40.0f);
    UIComponent_SetFlexDirection(title_container, LAYX_FLEX_DIRECTION_ROW);

    // 创建标题文本
    g_title_text = (ui_component_t *)UIText_Create("SELECT MAP",
                                                   (COLOR32){255, 215, 0, 255}, 30.0f,
                                                   UI_TEXT_ALIGN_LEFT, UI_TEXT_VALIGN_MIDDLE, g_ctx);
    if (!g_title_text) {
        printf("Failed to create title text\n");
        return -1;
    }
    UIContainer_AddChild((ui_container_t *)title_container, g_title_text);
    UIComponent_SetSize(g_title_text, 700.0f, 40.0f);
    UIComponent_SetAlignSelf(g_title_text, LAYX_ALIGN_SELF_FLEX_START);
    UIComponent_SetMargin(g_title_text, 10.0f, 0.0f, 0.0f, 10.0f);

    // 2. 创建路径容器（第二行）
    ui_component_t *path_container = (ui_component_t *)UIContainer_Create(1024.0f, 30.0f,
        MAKE(COLOR32, 0, 0, 0, 0),  // 透明背景
        MAKE(COLOR32, 0, 0, 0, 0),
        g_ctx
    );
    if (!path_container) {
        printf("Failed to create path container\n");
        return -1;
    }
    UIContainer_AddChild((ui_container_t *)g_root_container, path_container);
    UIComponent_SetSize(path_container, 1024.0f, 30.0f);
    UIComponent_SetFlexDirection(path_container, LAYX_FLEX_DIRECTION_ROW);

    // 创建路径文本
    g_path_text = (ui_component_t *)UIText_Create("Root",
    ...
    UIContainer_AddChild((ui_container_t *)path_container, g_path_text);
    UIComponent_SetSize(g_path_text, 1024.0f, 30.0f);
    UIComponent_SetAlignSelf(g_path_text, LAYX_ALIGN_SELF_FLEX_START);
    UIComponent_SetMargin(g_path_text, 10.0f, 0.0f, 0.0f, 10.0f);

    // 3. 创建内容容器（第三行，左右分栏）
    ui_component_t *content_container = (ui_component_t *)UIContainer_Create(1024.0f, 698.0f,
        MAKE(COLOR32, 0, 0, 0, 0),  // 透明背景
        MAKE(COLOR32, 0, 0, 0, 0),
        g_ctx
    );
    if (!content_container) {
        printf("Failed to create content container\n");
        return -1;
    }
    UIContainer_AddChild((ui_container_t *)g_root_container, content_container);
    UIComponent_SetSize(content_container, 1024.0f, 698.0f);
    UIComponent_SetFlexDirection(content_container, LAYX_FLEX_DIRECTION_ROW);

    // 4. 创建左侧容器（搜索+列表）
    ui_component_t *left_container = (ui_component_t *)UIContainer_Create(380.0f, 698.0f,
        MAKE(COLOR32, 0, 0, 0, 0),  // 透明背景
        MAKE(COLOR32, 0, 0, 0, 0),
        g_ctx
    );
    if (!left_container) {
        printf("Failed to create left container\n");
        return -1;
    }
    UIContainer_AddChild((ui_container_t *)content_container, left_container);
    UIComponent_SetSize(left_container, 380.0f, 698.0f);
    UIComponent_SetFlexDirection(left_container, LAYX_FLEX_DIRECTION_COLUMN);

    // 创建筛选标签
    g_filter_label = (ui_component_t *)UILabel_Create(50.0f, 28.0f, "Filter:",
    ...
    UIContainer_AddChild((ui_container_t *)left_container, g_filter_label);
    UIComponent_SetSize(g_filter_label, 360.0f, 28.0f);
    UIComponent_SetAlignSelf(g_filter_label, LAYX_ALIGN_SELF_FLEX_START);
    UIComponent_SetMargin(g_filter_label, 10.0f, 0.0f, 5.0f, 10.0f);

    // 创建筛选输入框
    g_filter_input = (ui_component_t *)UIInput_Create(360.0f, 28.0f, 14.0f, "Type to filter...", g_ctx);
    if (!g_filter_input) {
    ...
    UIComponent_SetAlignSelf(g_filter_input, LAYX_ALIGN_SELF_FLEX_START);
    UIComponent_SetMargin(g_filter_input, 10.0f, 0.0f, 5.0f, 0.0f);

    mgr->focused = g_filter_input;

    // 创建 UI 列表
    g_ui_list = (ui_component_t *)UIList_Create(360.0f, 650.0f, 50.0f, 14.0f, g_ctx);
    if (!g_ui_list) {
    ...
    UIComponent_SetAlignSelf(g_ui_list, LAYX_ALIGN_SELF_FLEX_START);
    UIComponent_SetMargin(g_ui_list, 10.0f, 0.0f, 10.0f, 0.0f);

    // 5. 创建右侧容器（预览+按钮）
    ui_component_t *right_container = (ui_component_t *)UIContainer_Create( 644.0f, 698.0f,
        MAKE(COLOR32, 0, 0, 0, 0),  // 透明背景
        MAKE(COLOR32, 0, 0, 0, 0),
        g_ctx
    );
    if (!right_container) {
        printf("Failed to create right container\n");
        return -1;
    }
    UIContainer_AddChild((ui_container_t *)content_container, right_container);
    UIComponent_SetSize(right_container, 644.0f, 698.0f);
    UIComponent_SetFlexDirection(right_container, LAYX_FLEX_DIRECTION_COLUMN);

    // 创建地图预览容器
    g_preview_container = (ui_component_t *)UIContainer_Create( 624.0f, 0.0f,
    ...
    UIComponent_SetAlignSelf(g_preview_container, LAYX_ALIGN_SELF_STRETCH);
    UIComponent_SetMargin(g_preview_container, 10.0f, 0.0f, 10.0f, 0.0f);

    // 将预览容器添加到右侧容器
    UIContainer_AddChild((ui_container_t *)right_container, g_preview_container);

    // 创建预览文本组件（标题 "Map Preview"）
    g_preview_title_text = (ui_component_t *)UIText_Create("Map Preview",
    ...
    UIComponent_SetSize(g_preview_title_text, 604.0f, 18.0f);
    UIComponent_SetAlignSelf(g_preview_title_text, LAYX_ALIGN_SELF_STRETCH);
    UIComponent_SetMargin(g_preview_title_text, 10.0f, 10.0f, 5.0f, 0.0f);
    
    // 创建预览文本组件（文件名）
    g_preview_filename_text = (ui_component_t *)UIText_Create("",
                                                              (COLOR32){200, 200, 200, 255}, 16.0f,
                                                              UI_TEXT_ALIGN_LEFT, UI_TEXT_VALIGN_TOP, g_ctx);
    ...
    UIComponent_SetAlignSelf(g_preview_filename_text, LAYX_ALIGN_SELF_STRETCH);
    UIComponent_SetMargin(g_preview_filename_text, 5.0f, 10.0f, 5.0f, 0.0f);
    
    // 创建预览文本组件（地图名称）
    g_preview_name_text = (ui_component_t *)UIText_Create("",
                                                         (COLOR32){180, 180, 180, 255}, 16.0f,
                                                         UI_TEXT_ALIGN_LEFT, UI_TEXT_VALIGN_TOP, g_ctx);
    ...
    UIComponent_SetAlignSelf(g_preview_name_text, LAYX_ALIGN_SELF_STRETCH);
    UIComponent_SetMargin(g_preview_name_text, 5.0f, 10.0f, 5.0f, 0.0f);
    
    // 创建预览文本组件（作者）
    g_preview_author_text = (ui_component_t *)UIText_Create("",
                                                         (COLOR32){160, 160, 160, 255}, 16.0f,
                                                         UI_TEXT_ALIGN_LEFT, UI_TEXT_VALIGN_TOP, g_ctx);
    ...
    UIComponent_SetAlignSelf(g_preview_author_text, LAYX_ALIGN_SELF_STRETCH);
    UIComponent_SetMargin(g_preview_author_text, 5.0f, 10.0f, 5.0f, 0.0f);
    
    // 创建预览文本组件（推荐玩家数）
    g_preview_players_text = (ui_component_t *)UIText_Create("",
                                                           (COLOR32){140, 140, 140, 255}, 16.0f,
                                                           UI_TEXT_ALIGN_LEFT, UI_TEXT_VALIGN_TOP, g_ctx);
    ...
    UIComponent_SetAlignSelf(g_preview_players_text, LAYX_ALIGN_SELF_STRETCH);
    UIComponent_SetMargin(g_preview_players_text, 5.0f, 10.0f, 5.0f, 0.0f);
    
    // 创建预览文本组件（文件类型）
    g_preview_type_text = (ui_component_t *)UIText_Create("",
                                                        (COLOR32){150, 150, 150, 255}, 16.0f,
                                                        UI_TEXT_ALIGN_LEFT, UI_TEXT_VALIGN_TOP, g_ctx);
    ...
    UIComponent_SetAlignSelf(g_preview_type_text, LAYX_ALIGN_SELF_STRETCH);
    UIComponent_SetMargin(g_preview_type_text, 5.0f, 10.0f, 5.0f, 0.0f);
    
    // 创建预览文本组件（完整路径）
    g_preview_path_text = (ui_component_t *)UIText_Create("",
                                                      (COLOR32){100, 100, 100, 255}, 14.0f,
                                                      UI_TEXT_ALIGN_LEFT, UI_TEXT_VALIGN_TOP, g_ctx);
    ...
    UIComponent_SetAlignSelf(g_preview_path_text, LAYX_ALIGN_SELF_STRETCH);
    ...
    ui_component_t *hint_container = (ui_component_t *)UIContainer_Create(644.0f, 170.0f,
        MAKE(COLOR32, 0, 0, 0, 0),  // 透明背景
        MAKE(COLOR32, 0, 0, 0, 0),
        g_ctx
    );
    if (!hint_container) {
        printf("Failed to create hint container\n");
        return -1;
    }
    UIContainer_AddChild((ui_container_t *)right_container, hint_container);
    UIComponent_SetSize(hint_container, 644.0f, 170.0f);
    UIComponent_SetFlexDirection(hint_container, LAYX_FLEX_DIRECTION_COLUMN);
    // hint_container不需要flex，保持固定高度

    // 创建提示文本1
    g_hint_text1 = (ui_component_t *)UIText_Create("UP/DOWN: navigate  ENTER: select  Type: filter",
                                                   (COLOR32){200, 200, 200, 255}, 14.0f,
                                                   UI_TEXT_ALIGN_LEFT, UI_TEXT_VALIGN_MIDDLE, g_ctx);
    if (!g_hint_text1) {
    ...
    UIComponent_SetAlignSelf(g_hint_text1, LAYX_ALIGN_SELF_STRETCH);

    // 创建提示文本2
    g_hint_text2 = (ui_component_t *)UIText_Create("ESC: quit",
                                                   (COLOR32){200, 200, 200, 255}, 14.0f,
                                                   UI_TEXT_ALIGN_RIGHT, UI_TEXT_VALIGN_MIDDLE, g_ctx);
    if (!g_hint_text2) {
    ...
    UIComponent_SetAlignSelf(g_hint_text2, LAYX_ALIGN_SELF_STRETCH);
    ...
    g_start_button = (ui_component_t *)UIButton_CreateWithConfig(250.0f, 50.0f, &button_config, g_ctx);
    if (!g_start_button) {
    ...
    UIComponent_SetAlignSelf(g_start_button, LAYX_ALIGN_SELF_STRETCH);
...
static void ApplyLayoutToComponents(void) {
    printf("ApplyLayoutToComponents called\n");
...
void MapSelect_Shutdown(void) {
    ...
    // 清理lay布局上下文
    layx_destroy_context(g_lay_ctx);
...
int MapSelectScene_Init(scene_t *scene, const scene_params_t *params) {
    ...
    // 调用原有的初始化函数（传递 scene 参数）
    int result = MapSelect_Init(scene);
    if (result != 0) {
